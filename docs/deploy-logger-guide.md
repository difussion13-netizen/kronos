...
2. В правом верхнем углу страницы — выпадающий список «регион» (географическая зона
   дата-центров; от неё зависит цена и скорость). Выберите **US East (N. Virginia)**.
   Почему не Ирландия: биржа Polymarket physically живёт в вирджинских дата-центрах
   Amazon; в Вирджинии и дешевле, и данные потом уйдут на GPU-сервер без доплат.
   Для записи логов регион роли не играет вообще, просто привыкаем к правильному месту.
...
бакете лежат файлы `*.jsonl.gz`. Дальше программа работает сама: перезапустится после
