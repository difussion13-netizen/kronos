...
- **Раз в неделю** (5 секунд): открыть `systemctl status kronolog` в Session Manager —
  ...
  ≈8.4 GB/день —по замеру 2026-09-07 (bench/audit_s3.py): 3.9 суток = 32.4 GB, почти всё — поток clob;
  при 150 днях накопления это ~1.25 TB ≈ $29/мес Standard; можно сэкономить ~2×,
  настроив на бакет life-cycle перевод в S3 Standard-IA через 60 дней
  (aws s3api put-bucket-lifecycle-configuration — или скажи мне, впишу).
